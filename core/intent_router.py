...
persona_style: use nigerian_twitter unless they sound formal.
"""
...
def _extract_item_from_query(query: str) -> tuple[str, str]:
    m = re.search(r"\b(?:review|rate)\s+(?:for|about|on)\s+(.+?)(?:[.?!]|$)", query, re.I)
    if m:
...
def _default_candidates_from_query(query: str, interests: list[str]) -> list[str]:
    ...
    pool.extend(["Weekend outing idea", "Under-10k gift bundle", "Rainy-day comfort pick"])
    for i in interests:
        if i and i not in pool:
            pool.append(f"{i.title()} starter pack")
    dedup: list[str] = []
    seen: set[str] = set()
    for p in pool:
        k = p.lower()
...
def heuristic_route(query: str, persona_interests: list[str]) -> RouterDecision:
    q = query.lower().strip()
    rec_signals = (
        "recommend",
        "suggest",
    ...
    if any(s in q for s in rec_signals) and not _looks_like_explicit_review_request(q):
        return RouterDecision(
            ...
            context=query,
            persona_style=settings.default_persona_style,
        )
...
def route_query(user_persona: dict[str, Any], query: str) -> tuple[RouterDecision, Literal["llm", "heuristic"]]:
    ...
    return heuristic_route(query, interests), "heuristic"
